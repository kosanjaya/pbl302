...
@if(Auth()->user()->role == 'admin')
        <!-- Content Start -->
    <form action="{{ url('/push/data/'.$newData->id) }}" method="POST" enctype="multipart/form-data">
        @method('put')
        @csrf
        <div class="w-full border-b border-primary pb-4">
          ...
          <p class="text-white text-[1rem] font-sans font-thin tracking-wider">
            Update with Insights From the Uncovered Bug
          </p>
        </div>
        ...
        <div class="my-6">
          <p
            class="text-white text-[1.5rem] font-['Roboto'] font-normal italic tracking-widest"
          >
            #ReportID <span>{</span> {{ $newData->id }} <span>}</span>
            #email-user <span>{</span> {{ $newData->submitted_by }} <span>}</span>
          </p>
        </div>
        <!-- gap -->
        <div class="my-8">
        ...
        <div class="my-8">
          ...
          >
            ...
            <option value="E-Learning" @if($newData->asset_name == "E-learning") selected @endif>E-Learning</option>
            <option value="Silam" @if($newData->asset_name == "Silam")selected @endif>SILAM</option>
            <option value="SID" @if($newData->asset_name == "SID")selected @endif>SID</option>
            <option value="Polibatam.ac.id" @if($newData->asset_name == "Polibatam.ac.id") selected @endif>Polibatam.ac.id</option>
          </select>
        </div>
        <!-- gap -->
        ...
        <div class="my-4">
          ...
          <div class="flex gap-4">
            ...
            >
              ...
              <option value="low" @if($newData->severity == "low") selected @endif>LOW</option>
              <option value="medium" @if($newData->severity == "medium") selected @endif>MEDIUM</option>
              <option value="high" @if($newData->severity == "high") selected @endif>HIGH</option>
              <option value="critical" @if($newData->severity == "critical") selected @endif>CRITICAL</option>
            </select>
            <input
              class="w-[12rem] rounded-md input-bundle py-3 px-6"
              type="text"
              placeholder="score (ex:8.2)"
              value="{{ $newData->severity_score }}"
              name = "severity_score"
              id = "severity_score"
            />
          </div>
        ...
        <div class="my-8">
          <p
            class="text-white text-[1rem] font-sans font-normal tracking-wider mb-2"
          >
            Proof of Concept (ex: Screenshot) :
          </p>
          <input
